...
base. Each of the eight tabs in the UI shows one Redis capability —
exact vector search, approximate vector search, hybrid semantic
search, semantic caching, semantic routing, Bloom-filter + vector
duplicate-identity detection, concurrent throughput, and live schema
introspection — plus a persistent explanation of exactly which Redis
command or RedisVL feature is producing that result. Python/FastAPI
backend, single-file vanilla frontend, Redis in Docker.
...
- **RedisBloom:** raw `BF.RESERVE`/`BF.MADD`/`BF.EXISTS`/`BF.INFO`
  commands via `execute_command()` — not a RedisVL scenario, since
  RedisVL doesn't wrap probabilistic data structures. Ships built into
  the `redis:8` image already used here; no extra service needed.
...
**`customer_idx` embeds the customer's NAME ONLY — city/state are a
`Tag("city")` filter in `check_duplicate()`, never blended into the
embedded text.** An earlier version embedded `"<name>, <city>, <state>"`
and measured *worse* false-positive behavior for it: two unrelated
people sharing both a last name and a city (e.g. "Robert Smith" and
"Daniel Smith," same city) land closer together when they also share a
city token on top of the last-name token, not farther apart. Don't
"simplify" this back to one blended string — see docs/METHODOLOGY.md
for the full measurement, including the honest finding that even with
this fix, the true-positive and false-positive distributions still
overlap in the tail (measured: 98% recall at a 0.56% false-positive
rate, not 100%/0%) — `NEAR_DUPLICATE_DISTANCE_THRESHOLD` in `config.py`
is a documented tradeoff, not a bug to keep chasing to zero.

**`generate.py`'s `FIRST_NAMES`/`LAST_NAMES` pools are deliberately
larger than a first pass used.** A smaller pool (34/24) meant a
5,000-person corpus with 20 cities averaged ~10 people sharing any
given (last name, city) — too dense for the fuzzy duplicate-detection
layer to stay precise. Don't shrink these pools without re-running the
exhaustive same-last-name-same-city sweep described in
docs/METHODOLOGY.md to re-check the false-positive rate.
...
- `customer:<customer_id>` — Hash, indexed by `customer_idx` (FLAT,
  embedding on name only). Every customer here IS loaded (unlike the
  other corpora, which stay whole-corpus searches) because duplicate
  detection is inherently "does this match something already in the
  base" — there's nothing to demonstrate without the base loaded.
- `tin_seen` — a RedisBloom `BF.RESERVE` filter over every customer's
  TIN, checked via `BF.EXISTS` before anything else in
  `check_duplicate()`.
- `tin:<tin>` — a Set of customer IDs sharing that exact TIN, used to
  confirm (or catch a false positive from) a Bloom "maybe."

No real customer or case data of any kind is loaded — every TIN, name,
and case note is synthetic, generated by `src/generate.py` from a fixed
seed. See "What this demo does not support" in docs/METHODOLOGY.md for
what this demo doesn't attempt.
